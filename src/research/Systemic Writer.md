...
### Cycle Five: Theory Consolidation — Software Iterations and a Framework-Drafting Episode (April–May 2026)
...
Writing Zebra and Writing Orchestrator followed as short software iterations that returned to PySide6 desktop and implemented the pipeline/engine concept in clean form — validating the phase-pipeline pattern and producing a detailed system design document that specified the orchestration architecture with enough precision to drive subsequent implementation.

Running in parallel with the software iterations during this period was a separate development that requires its own characterization: the Reader-First prompt and framework document. This was an AI-assisted writing project in its own right — a book-length theoretical and methodological specification drafted iteratively, through its own independent revision cycle, largely separate from the design-experiment cadence. It evolved alongside the software work and absorbed what was being learned across multiple design cycles, but it was not itself a design experiment. It was authorship: the sustained conceptual work of translating accumulated understanding into a stable, transmissible framework.

What crystallized during this period was a complete theoretical account of what long-form writing is actually doing. The Reader-First model frames a piece of writing not as content delivery but as an intervention in the reader's attractor landscape — an attempt to shift a stable cognitive or emotional state toward a different one. This framing had been implicit in earlier iterations. Here it was made explicit, operational, and comprehensive enough to govern every phase of the software's design.
...
**Theory development and software design experimentation are distinct activities.** The ReaderFirst directory's nine numbered files, produced in a single day, were successive saves of a document in active drafting — not nine tested software implementations. Genuine design-experiment cycles in this project required days to weeks each; the signature of intensive document editing looks very different from that of iterative system testing. Conflating the two activities produces a distorted account of both the pace and the nature of the work. The Reader-First framework was developed as an extended authorship and framework-drafting episode (~3.5 weeks, April–May 2026), and is correctly counted as a single framework-development episode rather than nine design-experiment cycles. Keeping this distinction clear is both a matter of methodological accuracy and a finding transferable to other design experiments in complex domains: the theoretical work that eventually governs a software system is not the same kind of activity as the experimental cycles it informs, and the two should be planned, documented, and counted accordingly.
